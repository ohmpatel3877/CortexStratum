...
class ProjectSpec:
    ...
    def __init__(self):
        self.mandatory_subsystems = [
            'drive', 'power transmission', 'clamping', 'inverting',
            'sensing', 'control', 'frame'
        ]
...
def verify_scad_file(filepath, spec):
    ...
    has_frame = 'base' in content.lower() or 'frame' in content.lower() or 'rib' in content.lower()
...
def verify_assembly(scad_dir):
    ...
    print(f'Project Spec Verification - Bottle Flipping System')
    ...
    for req in spec.mandatory_subsystems:
        ...
        key_map = {
            'drive': 'servo', 'power transmission': 'gear',
            'clamping': 'clamp', 'inverting': 'servo',
            'sensing': None, 'control': None,
            'frame': 'frame'
        }
        key = key_map.get(req)
...
if __name__ == '__main__':
    path = sys.argv[1] if len(sys.argv) > 1 else 'G:/My Drive/Project/Bottle Flipper/cad'
