...
def search_grabcad(query, max_results=10):
    ...
    url = f"https://grabcad.com/library?page=1&time=all_time&sort=recent&query={encoded}"
...
def download_model(url, output_dir=None):
    ...
    try:
        ...
        if downloads_found:
            ...
            return {"model": model_name, "files": results, "output_dir": str(output_dir)}
        else:
            print("[grabcad] No direct download links found on page.")
            print("[grabcad] Model may require login or have download buttons behind JS.")
            return {"model": model_name, "error": "No downloadable links found", "url": url}
