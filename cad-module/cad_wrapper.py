...
def handle_tool_call(name, args):
    if name == "read_cad_validate_scad":
        ...
        issues = cv.validate_scad(args["filepath"])
        return {"filepath": args["filepath"], "issues": issues, "status": "valid" if not issues else "issues_found"}
        ...
        return {"stress_MPa": result[0], "safety_factor": result[1], "deflection_mm": result[2]}
