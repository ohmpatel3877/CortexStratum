...
def _load_tool_inventory():
    ...
    try:
        ...
        spec = _util.spec_from_file_location("tools_server", str(SCRIPTS_DIR / "tools-mcp-server.py"))
        if spec and spec.loader:
            mod = _util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            tools = getattr(mod, "TOOLS", [])
            inventory = {t["name"]: {"permission": t.get("permission"), "has_dry_run": "dry_run" in str(t.get("inputSchema", {}))} for t in tools}
...
def assess_scope(trigger_text=""):
    ...
    domain_keywords = {
        ...
        "simulation": ["simulate", "fea", "cfd", "stress", "buckle", "fatigue", "mechanics", "beam"],
        "compact": ["compact", "compress", "condense", "token", "velocity"],
        "sensory": ["browse", "scrape", "screenshot", "pdf", "ocr", "web"],
        "code": ["code", "review", "analyze", "refactor", "debug", "python", "typescript"],
    ...
    domain_tool_map = {
        "memory": ["read_memory_search", "write_memory_add", "read_memory_synthesize", "write_memory_consolidate"],
        "simulation": ["read_sim_mech_stress", "read_sim_mech_buckle", "read_sim_mech_fatigue_sn", "read_sim_mech_fatigue_goodman"],
        "compact": ["read_compact_token_velocity", "read_compact_synthesize", "write_compact_execute"],
        "sensory": ["read_sensory_browse", "read_sensory_screenshot", "read_sensory_extract_pdf"],
        "code": ["read_coder_analyze_code", "read_coder_review", "read_coder_debug"],
        "pipeline": ["read_verifier_status", "read_compact_status"],
        "system": ["read_xtrace_status", "read_goal_registry_status", "read_audit_status"],
    }
    suggested_tools = []
    ...
    return {
        ...
        "trigger_analyzed": trigger_text[:200] + ("..." if len(trigger_text) > 200 else ""),
...
def audit_redundancy(scope_id="", domains=None):
    ...
    if isinstance(inventory, dict) and "error" in inventory:
        return {"error": inventory["error"], "note": "Run tools-mcp-server.py first to build inventory"}
    ...
    module_map = {
        "xtrace": "trace.py", "dtrace": "trace.py", "goal_registry": "trace.py",
        "commitment_checker": "trace.py",
        "compact": "compact-module.py",
        "sim_mech": "sim-mechanics-module.py",
        "sensory": "sensory-module.py", "audio": "audio-module.py",
        "coder": "coder-module.py", "devops": "devops-module.py",
        "gamedev": "game-dev-module.py",
        "memory": "memory_search.py",
        "verifier": "verifier_middleware.py",
        "hooks": "hooks.py",
        "art": "art-module.py", "lit": "literature-module.py",
    ...
    return {
        ...
        "redundancy_clusters": {k: v for k, v in list(redundancy_clusters.items())[:10]},
        "cluster_count": len(redundancy_clusters),
        "unlinked_tools": unlinked[:10],
        "unlinked_count": len(unlinked),
        "scope_id": scope_id,
        "audit_result": "PASS" if len(unlinked) == 0 else f"{len(unlinked)} tools with unrecognized module mapping",
        "recommendation": "No action needed" if len(unlinked) == 0 and len(redundancy_clusters) <= 2 else "Review flagged clusters for consolidation opportunities"
    }
...
def execute_mutation(scope_id="", dry_run=False):
    ...
    if audit.get("cluster_count", 0) > 2:
        for cluster_name, tools in list(audit.get("redundancy_clusters", {}).items())[:5]:
            refactors.append({
                "type": "consolidation",
                "target": cluster_name,
    ...
    mutation = {
        "mutation_id": hashlib.md5(f"{scope_id}{time.time()}".encode()).hexdigest()[:12],
        "timestamp": time.time(),
        "scope_id": scope_id,
        "audit": {
...
def handle_tool_call(name, args):
    if name == "read_mutate_scope":
        ...
        return audit_redundancy(
            args.get("scope_id", ""),
            args.get("domains")
        )
    elif name == "write_mutate_execute":
