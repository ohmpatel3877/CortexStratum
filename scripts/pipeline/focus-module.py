...
def check_scope(input_text, current_project="CortexStratum"):
    ...
    for proj in KNOWN_PROJECTS:
        if (
            proj.lower() in input_text.lower()
            and proj.lower() != current_project.lower()
        ):
    ...
    return {
        ...
        "reasons": reasons,
        "overall_risk": "high"
        if classification in ("bloated", "cross_project")
        else ("medium" if classification == "scope_creep" else "low"),
    }
...
def generate_nudge(scope_result, user_input):
    ...
    if classification == "scope_creep":
        message = (
            "I notice we\u2019re drifting from the original goal. "
            "Let me log this as a future spec so we can stay focused on what matters now."
        )
        return {
            "message": message,
            "suggested_action": "write_future_spec",
...
def read_global(project=None, limit=10):
    ...
    if project:
        entries = [
            e for e in entries if e.get("project", "").lower() == project.lower()
        ]
...
def advance_pipeline(next_phase, summary=""):
    ...
    if next_phase not in VALID_PHASES:
        return {
            "error": f"Invalid phase: {next_phase}. Valid phases: {', '.join(VALID_PHASES)}"
        }
        ...
        for dep_marker in t.get("dependencies", []):
            for other in scored:
                if other["id"] != tid:
                    ...
                    if dep_marker in desc_lower or dep_marker in other.get(
                        "dependencies", []
                    ):
...
def learn(session_id, events=None):
    ...
    if scope_creep_events > 0:
        what_caused_switches.append(
            f"{scope_creep_events} out-of-scope tasks saved to Global Memory"
        )
    if len(project_tasks) > 1:
        what_caused_switches.append(
            f"Tasks span {len(project_tasks)} different projects"
        )
    ...
    if session_duration and session_duration > 3600:
        recommendations.append(
            "Session exceeded 1 hour \u2014 consider shorter focused sessions"
        )
    if len(phase_names) < 2:
        recommendations.append(
            "Try completing more pipeline phases for better session structure"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        store = store_global(
            "StudySpace", "Build dashboard", "From scope check", "test-001"
        )
