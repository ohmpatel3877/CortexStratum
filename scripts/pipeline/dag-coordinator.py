...
def _simulate_node_execution(nid, node_def, input_data, timeout_s):
    ...
    output = {
        "summary": f"Completed: {input_data.get('task', node_def['description'])[:80]}",
        "data": {},
        "artifacts": [],
        "files_changed": [],
        "decisions": [],
...
def execute_pipeline(dag, dry_run=False, resume=False, task_input=None):
    ...
    for node in dag["nodes"]:
        ...
        mgr.init_node_state(
            nid,
            {
                "task": node.get("prompt_template", node["description"]),
                "context": dict(root_input),
            },
            node.get("temperature", 0.7),
        )
    ...
    for level_idx, level in enumerate(levels):
        ...
        for nid in level:
                ...
                for edge in dag.get("edges", []):
                    if (
                        ...
                        if not _evaluate_condition(edge["condition"], dep_state):
                            ...
                            mgr.mark_skipped(
                                nid, f"Condition {edge['condition']} not met"
                            )
                            skip_node = True
                            break
                if skip_node:
            ...
            merged_input = _build_node_input(
                nid, node_def, upstream_outputs, root_input
            )
            mgr.write_state(nid, {**mgr.read_state(nid), "input": merged_input})
            mgr.mark_running(nid)

            node_start = time.time()
            node_result = _simulate_node_execution(
    ...
    final_status = (
        "failed"
        if failed_nodes
        else ("timeout" if trace["status"] == "timeout" else "completed")
    )
    trace["completed_at"] = now_iso()
    ...
    trace_path = os.path.join(
        DAG_TRACE_DIR, f"{dag_id}-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
    )
    ...
    if post_verify:
        ...
        try:
            result = subprocess.run(
                post_verify.split(), capture_output=True, text=True, timeout=30
            )
    ...
    if os.path.isfile(server_script):
        try:
            ...
            if result.returncode == 0:
                ...
                if expected:
                    found = sum(
                        1 for t in tools if any(e in t["name"] for e in expected)
                    )
                    print(
                        f"  {C}    Expected tools matched: {found}/{len(expected)}{N}"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Validate + print plan, no execution"
    )
    parser.add_argument(
        "--resume", action="store_true", help="Skip already-completed nodes"
    )
        ...
        for fname in sorted(os.listdir(DAG_DEF_DIR)):
            if fname.endswith(".json"):
                ...
                try:
                    d = load_json(fpath)
                    print(
                        f"  {fname}  ({d.get('dag_id', '?')})  {d.get('name', '?')}  [{len(d.get('nodes', []))} nodes]"
                    )
    ...
    execute_pipeline(
        dag, dry_run=args.dry_run, resume=args.resume, task_input=task_input
    )
