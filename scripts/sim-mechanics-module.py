...
def beam_deflection_point_load(load, length, E, I, position=None):
    ...
    """
    if E == 0 or I == 0:
        return {"error": "Young's modulus (E) and moment of inertia (I) must be non-zero"}
    ...
    if position is None:
        ...
        if x <= a:
            ...
            deflection = load * a * (length - x) * (2 * length * x - x**2 - a**2) / (6 * E * I * length)
...
def column_euler_buckling(E, I, K, L):
    ...
    slenderness = L / math.sqrt(I / (math.pi * (I * 4 / math.pi)**0.5)) if I > 0 else 0
    return {
        ...
        "buckling_regime": "Euler (long column)" if slenderness > 100 else "Transitional",
        "units": "N (Newtons)"
    }
...
def fatigue_goodman_correction(S_alt, S_mean, S_ut, S_e):
    ...
    return {
        ...
        "status": "SAFE" if safe else f"FAILS (exceeds by {round((ratio-1)*100, 1)}%)",
        "equivalent_alternating_pa": S_alt / (1 - S_mean / S_ut) if S_mean < S_ut else float('inf'),
...
def fatigue_miner_rule(blocks):
    ...
    return {
        ...
        "status": "FAILED (D >= 1)" if D >= 1 else f"SAFE (D < 1, remaining life factor: {round(1/D - 1, 4) if D > 0 else 'inf'})",
...
def handle_tool_call(name, args):
    ...
    if name == "read_sim_mech_moi":
        ...
        return beam_moment_of_inertia_rect(
            args.get("width", 0),
            args.get("height", 0)
        )
    ...
    # Backward compat: old fatigue/buckle/MOI names still work
        ...
        return column_johnson_buckling(
            args.get("E", 0), args.get("sigma_y", 0),
            args.get("A", 0), args.get("K", 1.0),
            args.get("L", 0), args.get("r", 0)
        )
    elif name == "read_sim_mech_fatigue_sn":
        return fatigue_sn_curve(args.get("Sf_prime", 0), args.get("b", -0.1), args.get("N", 1000))
    elif name == "read_sim_mech_fatigue_cycles":
        return fatigue_cycles_to_failure(args.get("Sf_prime", 0), args.get("b", -0.1), args.get("stress_amplitude", 0))
        ...
        return fatigue_goodman_correction(
            args.get("S_alt", 0), args.get("S_mean", 0), args.get("S_ut", 0), args.get("S_e", 0)
        )
    elif name == "read_sim_mech_fatigue_miner":
        return fatigue_miner_rule(args.get("blocks", []))
    elif name == "read_sim_mech_fastener_shear":
        return fastener_shear(args.get("force", 0), args.get("area", 0), args.get("num_fasteners", 1))
    elif name == "read_sim_mech_bolt_torque":
        return bolt_torque_preload(args.get("K", 0.2), args.get("D", 0), args.get("F", 0))
    elif name == "read_sim_mech_bonded_joint":
        return bonded_joint_stress(args.get("force", 0), args.get("width", 0), args.get("overlap_length", 0))
