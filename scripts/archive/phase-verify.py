...
check(
    "1",
    "artifact protection",
    s["protected_blocks"] == 1,
    f"{s['protected_blocks']} blocks",
)
...
beam_bending_stress = mech.beam_bending_stress
beam_shear_stress = mech.beam_shear_stress
beam_deflection_point_load = mech.beam_deflection_point_load
column_euler_buckling = mech.column_euler_buckling
column_johnson_buckling = mech.column_johnson_buckling
fatigue_sn_curve = mech.fatigue_sn_curve
fatigue_cycles_to_failure = mech.fatigue_cycles_to_failure
fatigue_goodman_correction = mech.fatigue_goodman_correction
fatigue_miner_rule = mech.fatigue_miner_rule
fastener_shear = mech.fastener_shear
bolt_torque_preload = mech.bolt_torque_preload
...
check(
    "4.4",
    "bending stress",
    abs(stress["stress_mpa"] - 4.167) < 0.01,
    f"{stress['stress_mpa']:.2f} MPa",
)
...
check(
    "4.4",
    "deflection center",
    deflect["deflection_mm"] > 0,
    f"{deflect['deflection_mm']:.3f} mm",
)
...
check(
    "4.4",
    "Euler buckling",
    buckle["critical_load_kN"] > 0,
    f"{buckle['critical_load_kN']:.2f} kN",
)
...
check(
    "4.4",
    "Johnson buckling",
    "critical_load_N" in johnson,
    f"{johnson.get('critical_load_kN', 0):.2f} kN",
)
...
check(
    "4.4",
    "S-N curve",
    sn["stress_amplitude_pa"] > 0,
    f"{sn['stress_amplitude_mpa']:.2f} MPa",
)
...
check(
    "4.4",
    "cycles to failure",
    Nf["cycles_to_failure"] > 0,
    f"{Nf['cycles_to_failure']} cycles",
)

goodman = fatigue_goodman_correction(100e6, 50e6, 400e6, 200e6)
check(
    "4.4", "Goodman safe", goodman["safe"] is True, f"ratio={goodman['goodman_ratio']}"
)

miner = fatigue_miner_rule(
    [
        {"cycles": 1000, "cycles_to_failure": 5000},
        {"cycles": 2000, "cycles_to_failure": 10000},
    ]
)
check(
    "4.4",
    "Miner rule",
    miner["cumulative_damage_D"] > 0,
    f"D={miner['cumulative_damage_D']}",
)
...
check(
    "4.4",
    "fastener shear",
    abs(fs["shear_stress_mpa"] - 12.5) < 0.1,
    f"{fs['shear_stress_mpa']:.2f} MPa",
)
...
check(
    "4.4",
    "bonded joint",
    bond["shear_stress_pa"] > 0,
    f"{bond['shear_stress_kpa']:.2f} kPa",
)
...
assess_scope = mutmod.assess_scope
audit_redundancy = mutmod.audit_redundancy
execute_mutation = mutmod.execute_mutation
get_status = mutmod.get_status

scope = assess_scope("debug memory consolidation with compaction")
check(
    "2",
    "scope domains detected",
    len(scope["detected_domains"]) > 0,
    str(scope["detected_domains"]),
)
...
check(
    "2",
    "audit total tools",
    audit["total_tools"] >= 60,
    f"{audit['total_tools']} (cache may be stale — run --list-tools to refresh)",
)
