...
def _format_matrix_latex(A, name="A"):
    ...
    return f"{name} = \\begin{{bmatrix}}{'\\\\\\\\'.join(rows)}\\end{{bmatrix}}"
...
def ode_solve(derivatives, y0, t_start=0.0, t_end=10.0, steps=100, method="rk4"):
    ...
    def _make_func(expr):
        def f(t, y):
            ...
            try:
                return eval(expr, {"math": math, "t": t}, {f"x{i+1}": y[i] for i in range(len(y))})
            except:
                return 0
        return f
