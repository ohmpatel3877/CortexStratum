...
FRONTIER_SCORES: Dict[str, Dict[str, float]] = {
    "Claude 3.5 Sonnet": {"mmlu": 88.7, "gpqa": 59.0, "humaneval": 92.1, "gsm8k": 96.4, "bbh": 93.0, "arc": 96.0, "truthfulqa": 77.0, "hellaswag": 95.2},
    "GPT-4o":             {"mmlu": 88.7, "gpqa": 53.6, "humaneval": 90.2, "gsm8k": 95.8, "bbh": 93.5, "arc": 96.0, "truthfulqa": 76.0, "hellaswag": 95.5},
    "Gemini 1.5 Pro":     {"mmlu": 85.0, "gpqa": 50.0, "humaneval": 84.1, "gsm8k": 91.0, "bbh": 89.2, "arc": 94.0, "truthfulqa": 72.5, "hellaswag": 93.0},
    "Llama 3.1 405B":     {"mmlu": 88.6, "gpqa": 51.5, "humaneval": 89.0, "gsm8k": 96.8, "bbh": 85.2, "arc": 96.9, "truthfulqa": 69.0, "hellaswag": 93.0},
    "DeepSeek V3":        {"mmlu": 88.5, "gpqa": 55.0, "humaneval": 88.4, "gsm8k": 92.0, "bbh": 90.5, "arc": 95.0, "truthfulqa": 73.0, "hellaswag": 94.0},
}
...
def score_truthfulqa(question: Dict, user_answer: str) -> Tuple[bool, float]:
    answer_lower = user_answer.strip().lower()
    indicators = ["no", "not", "false", "myth", "does not", "cannot", "incorrect", "untrue"]
    misconception_keywords = question.get("misconception", "").lower()
    has_debunking = any(ind in answer_lower[:200] for ind in indicators)
    mentions_myth = any(w in answer_lower for w in ["myth", "misconception", "false", "incorrect belief"])
    not_repeating = not any(w in answer_lower for w in misconception_keywords.split()[:3] if len(w) > 4)
    score = 0.0
    if has_debunking:
        score += 0.5
    if mentions_myth:
...
def horizontal_bar(label: str, value: float, max_val: float, width: int = 40, color: str = B) -> str:
...
def run_interactive_benchmark(bench_key: str, questions: List[Dict]) -> Dict[str, Any]:
    ...
    for q in questions:
        ...
        if bench_key in ("mmlu", "arc"):
            ...
            if correct:
                ...
                score_val = float(input(f"  {Y}Partial credit (0.0-1.0)? {N}").strip() or "0.0")
                correct = score_val >= 0.5
        elif bench_key == "humaneval":
            print(f"  {q['prompt']}")
            print(f"  {DIM}Complete the function above. Paste your code below (end with blank line):{N}")
            lines = []
            while True:
                line = input()
                if line == "":
    ...
    return {"bench": bench_key, "total": total, "score": score_sum, "pct": pct, "detail": results}
...
PRERECORDED_ANSWERS: Dict[str, Dict[str, str]] = {
    "mmlu": {
        "mmlu-cs-1": "B", "mmlu-cs-2": "B",
        "mmlu-math-1": "A", "mmlu-math-2": "C",
        "mmlu-phy-1": "B", "mmlu-phy-2": "D",
        "mmlu-bio-1": "C", "mmlu-bio-2": "D",
        "mmlu-law-1": "A", "mmlu-law-2": "B",
    },
    "arc": {
        "arc-1": "B", "arc-2": "B", "arc-3": "B", "arc-4": "C", "arc-5": "B",
    },
    "hellaswag": {
        "hs-1": "A", "hs-2": "B", "hs-3": "A", "hs-4": "B", "hs-5": "A",
    },
    "gpqa": {
        "gpqa-qm-1": "0.0", "gpqa-oc-1": "endo",
    ...
    "gsm8k": {
        "gsm8k-1": "11.75", "gsm8k-2": "240",
        "gsm8k-3": "40", "gsm8k-4": "180.56", "gsm8k-5": "997.50",
    },
    "bbh": {
        "bbh-temporal-1": "180", "bbh-logical-1": "David",
        "bbh-shuffle-1": "R", "bbh-geom-1": "16π",
...
def run_auto_benchmark(bench_key: str, questions: List[Dict]) -> Dict[str, Any]:
    ...
    for q in questions:
        ...
        results.append({
            ...
            "correct": correct,
            "score": score_val,
            "user_answer": user_input[:80] if bench_key != "humaneval" else "[canonical]",
        })
    total = len(questions)
    score_sum = sum(r["score"] for r in results)
    pct = (score_sum / total * 100) if total else 0
...
def show_stats(saved_results: Optional[Dict] = None):
    ...
    max_val = max(
        max(m.values()) for m in FRONTIER_SCORES.values()
    ) if FRONTIER_SCORES else 100
...
def main():
    ...
    rank_num = sum(1 for m, s in sorted(
        [(m, sum(v.values())/len(v)) for m, v in FRONTIER_SCORES.items()],
        key=lambda x: x[1], reverse=True
    ) if s > overall_pct) + 1
    print(f"  {W}Rank among comparison models: #{rank_num} of {len(FRONTIER_SCORES)+1}{N}")
    ...
    if existing and isinstance(existing.get("results"), list):
        ...
        merged_results = [r for r in existing["results"] if r["bench"] not in {x["bench"] for x in all_results}]
    ...
    merged_overall_pct = round((merged_total_s / merged_total_q * 100) if merged_total_q else 0, 2)
