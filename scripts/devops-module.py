...
def container_debug(error_log: str, runtime: str = "podman",
                    context: str = "standalone") -> dict:
    """
        ...
        "permission denied": {
            ...
            "solutions": [
                ...
                {"fix": "Chown the host directory to the container UID",
                 "command": f"sudo chown -R CONTAINER_UID:CONTAINER_GID /host/path",
                 "risk": "safe"},
    ...
    if not matched:
        if "error" in error_log_lower or "failed" in error_log_lower or "exit code" in error_log_lower:
    ...
    return {
        "diagnosis": "\n".join(diagnosis) or "No specific pattern matched — review raw error output",
...
def permissions_analyze(mount_path: str, container_user: str = "",
                        host_user: str = "", error_symptom: str = "permission_denied") -> dict:
    """
...
def compose_generator(services: list, networks: list | None = None,
                      runtime: str = "docker") -> dict:
    """
    ...
    for svc in services:
        ...
        if healthcheck:
            ...
            test_cmd = healthcheck.get("test", "curl -f http://localhost:3000 || exit 1")
    ...
    if runtime == "podman":
        ...
        notes.append("Generated for Podman Compose — use podman-compose instead of docker-compose")
    else:
        notes.append("Generated for Docker Compose — use 'docker compose up -d' to start")
        notes.append("For multi-env: docker compose -f compose.yaml -f compose.prod.yaml up")
        podman_considerations = [
    ...
    if networks:
        notes.append(f"Networks defined: {', '.join(networks)} — ensure external creation if needed")
...
def samba_config(share_name: str, path: str, users: list | None = None,
                 guest_ok: bool = False, read_only: bool = False,
                 special_flags: list | None = None) -> dict:
    """
    ...
    share_section += f"  browseable = yes\n"
    ...
    setup_commands.append("sudo systemctl restart nmbd  # if NetBIOS name resolution needed")
...
def mergerfs_setup(source_paths: list, mount_point: str,
                   policy: str = "epmfs", options: dict | None = None) -> dict:
    """
    ...
    opt_parts = [
        f"defaults,allow_other,use_ino",
        f"policy={policy}",
...
def dockerfile_analyze(dockerfile: str) -> dict:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if re.search(r"(apt-get|apk|yum|dnf)\s+install", lower):
            install_saw = True
            if "clean" not in lower and "rm -rf" not in lower and "no-cache" not in lower:
                issues.append({
                    "line": i,
                    "severity": "info",
        ...
        if re.search(r"apt-get\s+(clean|autoremove)", lower) or "rm -rf /var/lib/apt/lists" in lower:
    ...
    if install_saw and not apt_clean_found:
        security_notes.append("Package cache not cleaned — use rm -rf /var/lib/apt/lists/* to reduce image size")
    ...
    if not healthcheck_found and ("web" in dockerfile.lower() or "server" in dockerfile.lower()):
        optimizations.append({
...
DEVOP_TOOLS = [
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "error_log": {"type": "string", "description": "The error log or message to analyze"},
                "runtime": {"type": "string", "enum": ["podman", "docker"], "default": "podman"},
                "context": {"type": "string", "enum": ["compose", "kubernetes", "standalone", "ci"], "default": "standalone"},
            },
            "required": ["error_log"],
        },
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "mount_path": {"type": "string", "description": "Path being mounted into the container (e.g., /data/media)"},
                "container_user": {"type": "string", "description": "UID:GID inside the container (e.g., 1000:1000)"},
                "host_user": {"type": "string", "description": "UID:GID on the host (e.g., 1001:1001)"},
                "error_symptom": {"type": "string", "enum": ["permission_denied", "ownership_wrong", "selinux_block"], "default": "permission_denied"},
            },
            "required": ["mount_path"],
        },
    },
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "share_name": {"type": "string", "description": "Name of the SMB share"},
                "path": {"type": "string", "description": "Absolute path to the share directory"},
                "users": {"type": "array", "items": {"type": "string"}, "description": "List of valid users for the share"},
                "guest_ok": {"type": "boolean", "default": False},
                "read_only": {"type": "boolean", "default": False},
                "special_flags": {"type": "array", "items": {"type": "string"}, "description": "VFS objects: vfs_fruit, time_machine, recycle, shadow_copy2, etc."},
            },
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "source_paths": {"type": "array", "items": {"type": "string"}, "description": "Source drive paths to pool"},
                "mount_point": {"type": "string", "description": "Target mount point for the pool"},
                "policy": {"type": "string", "enum": ["epmfs", "ff", "newest", "mfs", "lfs", "rand"], "default": "epmfs"},
                "options": {
                    "type": "object",
                    "properties": {
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "dockerfile": {"type": "string", "description": "Raw Dockerfile content to analyze"},
            },
            "required": ["dockerfile"],
        },
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "symptom": {"type": "string", "enum": ["cannot_reach_host", "dns_failure", "port_conflict", "bridge_not_working", "host_network_unavailable"]},
            },
            "required": ["symptom"],
        },
    },
