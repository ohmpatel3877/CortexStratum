...
BASE_DIR = Path(__file__).resolve().parent.parent.parent  # CortexStratum root
...
def run_powershell(
    script: str, args: List[str] = None, input_data: str = None
) -> Tuple[int, str, str]:
    ...
    proc = subprocess.run(
        cmd, capture_output=True, text=True, timeout=30, input=input_data, cwd=BASE_DIR
    )
...
def test_skill_router():
    ...
    test(
        "Skill Router: config is valid JSON",
        True,
        f"{len(config.get('rules', []))} rules",
    )
    ...
    test(
        "Skill Router: essential triggers present",
        len(missing) == 0,
        f"Missing: {missing}"
        if missing
        else f"Found all {len(required_triggers)} required triggers",
    )
    ...
    code, stdout, stderr = run_powershell(
        str(loader_path), ["-Message", "debug this electron IPC bug", "-DryRun"]
    )
    ...
    has_electron = (
        "electron-desktop-architecture" in stdout.lower()
        or "electron" in stdout.lower()
    )

    test(
        "Skill Router: matched debugging triggers",
        has_troubleshooting,
        stdout[:200] if not has_troubleshooting else "",
    )
    test(
        "Skill Router: matched electron triggers",
        has_electron,
        stdout[:200] if not has_electron else "",
    )

    # Test that default_skills always includes concise-filter
    defaults = config.get("default_skills", [])
    test(
        "Skill Router: defaults include concise-filter",
        "concise-filter" in defaults,
        str(defaults),
    )
...
def test_commitment_checker():
    ...
    test(
        "Commitment Checker: shows checklist format",
        "ACTIVE COMMITMENTS" in stdout or "commitment" in stdout.lower(),
        stdout[:200],
    )
    ...
    test(
        "Commitment Checker: has commitments",
        len(commitments) >= 5,
        f"{len(commitments)} found",
    )
    ...
    test(
        "Commitment Checker: covers all 5 behavioral fixes",
        len(found_texts) >= 3,
        f"Found: {found_texts}",
    )
...
def test_goal_registry():
    ...
    # Test Init
    code, stdout, stderr = run_powershell(
        str(goal_path), ["-Action", "Init", "-Goal", "Test goal registry functionality"]
    )
    test("Goal Registry: Init creates registry", code == 0, stderr if code else "")
    test(
        "Goal Registry: registry file exists",
    ...
    with open(registry_file) as f:
        data = json.load(f)
    test(
        "Goal Registry: sub_goals list populated",
        len(data.get("sub_goals", [])) >= 2,
        f"{len(data.get('sub_goals', []))} sub-goals",
    )

    # Test CompleteSubGoal
    code, stdout, stderr = run_powershell(
        str(goal_path), ["-Action", "CompleteSubGoal", "-Id", "0"]
    )
    ...
    test(
        "Goal Registry: sub-goal status updated",
    ...
    code, stdout, stderr = run_powershell(
        ...
        [
            ...
            "CheckAlignment",
            "-CurrentAction",
            "testing goal registry features",
        ],
    )
    test(
        "Goal Registry: CheckAlignment reports aligned",
        "ALIGNED" in stdout,
        stdout[:100],
    )
    ...
    test(
        "Goal Registry: Status shows output",
        code == 0 and len(stdout) > 50,
        stdout[:100],
    )
    test(
        "Goal Registry: Status shows original goal", "Test goal" in stdout, stdout[:100]
    )
...
def test_error_trace():
    ...
    code, stdout, stderr = run_powershell(
        ...
        [
            ...
            "npm run dev",
            "-ErrorOutput",
            "ERR_MODULE_NOT_FOUND: Cannot find module 'better-sqlite3'",
            "-ExitCode",
            "1",
        ],
    )
    ...
    test(
        "xTrace: registry file valid", len(errors) >= 2, f"{len(errors)} errors logged"
    )

    found = any("MODULE_NOT_FOUND" in e.get("error_signature", "") for e in errors)
    test(
        "xTrace: error signature captured correctly",
        found,
        f"Signatures: {[e.get('error_signature', '')[:50] for e in errors]}",
    )
    ...
    test(
        "xTrace: error status updated to resolved",
        len(resolved) >= 1,
    ...
    # Test Search
    code, stdout, stderr = run_powershell(
        str(trace_path), ["-Action", "Search", "-Keyword", "MODULE_NOT_FOUND"]
    )
    ...
    test(
        "xTrace: Status shows summary",
        code == 0 and ("error" in stdout.lower() or "Total" in stdout),
        stdout[:150],
    )
...
def test_decision_trace():
    ...
    code, stdout, stderr = run_powershell(
        ...
        [
            ...
            "-Context",
            "Building single-user Electron desktop app",
            "-Decision",
            "SQLite via better-sqlite3",
            "-Alternatives",
            "PostgreSQL;IndexedDB",
            "-Rationale",
            "SQLite ships with Electron, zero setup",
            "-Consequences",
            "No native replication;Locking with multiple writers",
            "-Category",
            "technology",
            "-Files",
            "src/main/database.ts;package.json",
        ],
    )
    ...
    test(
        "DTrace: registry has decisions",
        len(decisions) >= 1,
        f"{len(decisions)} decisions",
    )
    test(
        "DTrace: decision has all required fields",
        all(
            k in decisions[0]
            for k in ["id", "title", "decision", "rationale", "status"]
        ),
    ...
    # Test Search
    code, stdout, stderr = run_powershell(
        str(dtrace_path), ["-Action", "Search", "-Keyword", "SQLite"]
    )
    test(
        "DTrace: Search finds decisions",
        code == 0 and ("SQLite" in stdout or "sqlite" in stdout.lower()),
        stdout[:100],
    )

    # Test ByFile
    code, stdout, stderr = run_powershell(
        str(dtrace_path), ["-Action", "ByFile", "-FilePath", "src/main/database.ts"]
    )
    test(
        "DTrace: ByFile finds decisions",
        code == 0 and ("database.ts" in stdout or "SQLite" in stdout),
        stdout[:100],
    )
    ...
    test(
        "DTrace: Status shows summary",
        code == 0 and ("Total" in stdout or "decision" in stdout.lower()),
        stdout[:100],
    )
...
"""
    ...
    test(
        "Output Condenser: CondenseBash build succeeds",
        code == 0,
        stderr if code else "",
    )
    test(
        "Output Condenser: shows build completed",
        "Build completed" in stdout or "COMPLETED" in stdout or "CONDENSED" in stdout,
        stdout[:150],
    )
...
"""
    ...
    test(
        "Output Condenser: CondenseBash error succeeds",
        code == 0,
        stderr if code else "",
    )
...
"""
    code, stdout, stderr = run_powershell(
        str(condenser_path), ["-Command", "CondenseGrep", "-InputText", grep_output]
    )
    test("Output Condenser: CondenseGrep succeeds", code == 0, stderr if code else "")
    test(
        "Output Condenser: groups by file",
        "index.ts" in stdout and "preload" in stdout,
        stdout[:200],
    )
...
def test_cross_tool_integration():
    ...
    code3, _, _ = run_powershell(
        ...
        [
            ...
            "ERR_OSSL_EVP_UNSUPPORTED",
            "-RootCause",
            "Node.js v18+ OpenSSL3 incompatibility with Webpack 4",
            "-Resolution",
            "Set NODE_OPTIONS=--openssl-legacy-provider",
        ],
    )
    ...
    code4, _, _ = run_powershell(
        ...
        [
            ...
            "Add",
            "-Title",
            "Pin to Node.js v16 for build compatibility",
            "-Context",
            "Webpack 4 doesn't support OpenSSL3 in Node 18+",
            "-Decision",
            "Use .nvmrc with Node 16 for development",
            "-Alternatives",
            "Upgrade to Webpack 5;OpenSSL env var workaround",
            "-Rationale",
            "Node 16 is LTS and Webpack 4 is stable on it",
            "-Category",
            "process",
            "-Files",
            "package.json;.nvmrc",
    ...
    test(
        "Integration: xTrace + DTrace workflow",
        all_passed,
        f"Error log: {code1}, Attempt: {code2}, Resolve: {code3}, Decision: {code4}"
        if not all_passed
        else "",
    )
    ...
    error_resolved = any(
        e.get("status") == "resolved" for e in error_data.get("errors", [])
    )
    ...
    test(
        "Integration: commitment checker works after error workflow",
        code5 == 0,
        stdout5[:100] if code5 else "",
    )
...
def test_performance():
    ...
    tools = {
        "Skill Router": (
            SCRIPTS_DIR / "load-skills.ps1",
            ["-Message", "debug this bug", "-DryRun"],
        ),
        "Commitment Checker": (
            SCRIPTS_DIR / "check-commitments.ps1",
            ["-SessionStart"],
        ),
        ...
        "Output Condenser": (
            SCRIPTS_DIR / "output-condenser.ps1",
    ...
    for name, (script, args) in tools.items():
        ...
        test(
            f"Performance: {name} runs under 5s",
            elapsed < 5.0 and code == 0,
