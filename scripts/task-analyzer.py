...
def analyze_task(task: str) -> dict:
    ...
    file_patterns = len(
        re.findall(
            r"(?:file|module|component|class|function|route|endpoint)\b", task_lower
        )
    )
    ...
    return {
        ...
        "can_parallelize": mode in ("orchestrate", "pipeline")
        or independence_score >= 6,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Minimum score for parallel mode (default: 12)",
    )
    ...
    if args.file:
        ...
        try:
            with open(
                "C:\\Users\\ohmpa\\github\\CortexStratum\\data\\last-task.txt"
            ) as f:
                task = f.read()
