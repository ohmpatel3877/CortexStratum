...
light = [t for t in tools if not t.get("inputSchema",{}).get("required",[]) and "status" in t["name"]]
for t in light:
...
sim_mech = [t for t in tools if "sim_mech" in t["name"] and t["name"] not in (
    "read_sim_mech_fatigue", "read_sim_mech_buckle", "read_sim_mech_moi",
    "read_sim_mech_fatigue_goodman", "read_sim_mech_fatigue_miner"
...
sensory = [t for t in tools if "sensory" in t["name"] and "deprecated" not in t.get("description","").lower()]
...
print("  3. Merge force tools (deflection, shear, stress) into read_sim_mech_beam_analysis")
