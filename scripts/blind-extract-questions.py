...
extracted = {}
bench_names = {
    "MMLU_QUESTIONS": {"bench": "mmlu", "label": "MMLU (Multitask Language Understanding)"},
    "GPQA_QUESTIONS": {"bench": "gpqa", "label": "GPQA (Graduate-Level Q&A)"},
    "HUMANEVAL_QUESTIONS": {"bench": "humaneval", "label": "HumanEval (Code Generation)"},
    ...
    "TRUTHFULQA_QUESTIONS": {"bench": "truthfulqa", "label": "TruthfulQA (Truthfulness)"},
    "HELLASWAG_QUESTIONS": {"bench": "hellaswag", "label": "HellaSwag (Commonsense NLI)"},
}
...
if blind_match:
    try:
        ...
        print(f"  [OK] BLIND_QUESTIONS: {len(blind_questions)} questions (answers STRIPPED)")
    except:
        print(f"  [FAIL] BLIND_QUESTIONS: could not parse")
...
output = {
    ...
    "total_questions": sum(len(v["questions"]) for v in extracted.values()) + len(blind_questions),
