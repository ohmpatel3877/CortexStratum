...
def inspect_socket(host="localhost", port=None, path=None):
    ...
    if result["status"] == "REACHABLE":
        ...
        result["recommendation"] = f"Inspect and restart: {result.get('error', 'unknown error')}"
...
def trace_handoff(source=None, target=None, protocol_filter=None):
    ...
    # Filter by source
    if source:
        handoffs = {k: v for k, v in handoffs.items()
                    if source.lower() in v["source"].lower()}

    # Filter by target
    if target:
        handoffs = {k: v for k, v in handoffs.items()
                    if target.lower() in v["target"].lower()}
    ...
    return {
        ...
        "handoffs": results,
        "recommendation": "All monitored handoffs nominal" if results else "No handoffs to trace"
    }
...
def create_checkpoint(artifact_type="session", file_paths=None, metadata=None, dry_run=False):
    """
    ...
    checkpoint["prompt"] = (
        ...
        f"Proceed with mutation? (use write_mutate_execute to continue)"
    )
    ...
    if not dry_run:
        ...
        checkpoint["note"] = "Dry run. Execute without dry_run=true to persist checkpoint."
