# ADR-002: Identity/Persona Persistence via NE-Memory Distillation
...
NE-Memory (local agent-memory-mcp) stores user preferences, anti-patterns, and task learnings but has no mechanism for maintaining a consistent persona across sessions. Each session starts with a blank identity unless explicitly loaded via memory search. This leads to inconsistent behavior — the agent may act formal in one session and casual in another, or forget established principles.

The identity-manager.py script exists but operates standalone, not wired into the session startup flow.

## Decision

Distill identity from NE-Memory profiles at session start using a three-phase approach:

1. **Consolidation phase** — `scripts/identity-manager.py --consolidate` reads all NE-Memory entries and profile files, extracts traits (from `user_preferences`), principles (from `architecture_decisions`), behavioral patterns (from `anti_patterns`), and knowledge boundaries
...
2. **Full NE-Memory context injection** — too noisy, no structured abstraction
