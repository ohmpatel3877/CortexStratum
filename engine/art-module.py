...
def generate_svg(description: str, **kwargs) -> str:
    ...
    if "flowchart" in desc_lower or "diagram" in desc_lower:
        ...
        for i, step in enumerate(steps):
            ...
            row_y = y + (i // 2) * 80
            shapes.append(
                f'<rect x="{x}" y="{row_y}" width="130" height="40" rx="6" fill="#e2e8f0" stroke="#64748b" stroke-width="1.5"/>'
            )
            shapes.append(
                f'<text x="{x + 65}" y="{row_y + 25}" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#1e293b">{step}</text>'
            )
            if i < len(steps) - 1:
                sx, sy = x + 65, row_y + 40
                nx = 50 + ((i + 1) % 2) * 160
                ny = y + ((i + 1) // 2) * 80 + 20
                shapes.append(
                    f'<line x1="{sx}" y1="{sy}" x2="{nx}" y2="{ny}" stroke="#94a3b8" stroke-width="1.5" marker-end="url(#arrow)"/>'
                )
        ...
        for i, v in enumerate(values):
            ...
            shapes.append(
                f'<rect x="{x}" y="{y}" width="{bar_w}" height="{bh}" rx="3" fill="{colors[i % len(colors)]}" opacity="0.85"/>'
            )
            shapes.append(
                f'<text x="{x + bar_w / 2}" y="{h - 20}" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#475569">{labels[i]}</text>'
            )
            shapes.append(
                f'<text x="{x + bar_w / 2}" y="{y - 5}" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#1e293b">{v}</text>'
            )

    else:
        shapes.append(
            f'<text x="{w / 2}" y="40" text-anchor="middle" font-family="sans-serif" font-size="16" font-weight="bold" fill="#1e293b">{title}</text>'
        )
        shapes.append(
            f'<rect x="{(w - 200) / 2}" y="60" width="200" height="120" rx="8" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1"/>'
        )
        shapes.append(
            f'<text x="{w / 2}" y="130" text-anchor="middle" font-family="sans-serif" font-size="14" fill="#475569">Generated Illustration</text>'
        )
...
def extract_palette(base_color: str) -> dict:
    ...
    shades = [
        _hsl_to_hex(h, s, max(10, min(95, l + offset)))
...
if __name__ == "__main__":
    ...
    if action == "generate_svg":
        ...
        print(
            "Art Module — available actions: generate_svg, generate_theme, extract_palette, design_concept"
        )
