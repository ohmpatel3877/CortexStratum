...
def _extract_key_phrases(text: str, top_n: int = 15) -> list:
    ...
    return [
        {"word": w, "count": c, "score": round(c / len(filtered), 4)} for w, c in freqs
    ]
...
def analyze_text(text: str) -> dict:
    ...
    return {
        ...
        "sentiment": {
            ...
            else ("negative" if sentiment_polarity < -0.02 else "neutral"),
        },
...
def extract_concepts(text: str) -> dict:
    ...
    candidates = [
        c.strip() for c in candidates if len(c.strip()) > 3 and not c.strip().isdigit()
    ]
    ...
    for i in range(min(len(concepts), 8)):
        for j in range(i + 1, min(len(concepts), 8)):
            ...
            cooccur = sum(
                1
                for s in sentences
                if c1.lower() in s.lower() and c2.lower() in s.lower()
            )
            if cooccur > 0:
                relationships.append(
                    {
                        ...
                        "strength": "strong"
                        if cooccur > 3
                        else ("moderate" if cooccur > 1 else "weak"),
                    }
                )
    ...
    return {
        ...
        "concept_map": {
            "nodes": [
                {"id": c["concept"], "frequency": c["frequency"]} for c in concepts[:10]
            ],
            "edges": [
                {
                    "source": r["source"],
...
def generate_study_guide(content: str) -> dict:
    ...
    key_terms = [
        {"term": c["concept"], "definition": c["definition"]}
        for c in concepts_data.get("concepts", [])[:10]
    ]

    discussion_questions = []
    for c in key_terms[:5]:
        discussion_questions.append(
            f"How does the concept of '{c['term']}' relate to the main thesis of this text?"
        )
    discussion_questions.append(
        "What evidence does the author provide to support their central claim?"
    )
    discussion_questions.append(
        "What counterarguments could be raised against the main position?"
    )
    discussion_questions.append(
        "How does this content connect to broader themes in the field?"
    )
    discussion_questions.append(
        "What are the practical implications of the ideas presented?"
    )

    connections = concepts_data.get("relationships", [])[:5]
    connection_questions = []
    for rel in connections:
        connection_questions.append(
            f"Explore the relationship between {rel['source']} and {rel['target']} — how do they interact?"
        )
        ...
        "summary_by_section": [
            {
                ...
                "key_points": [
                    c["concept"]
                    for c in concepts_data.get("concepts", [])[i * 3 : (i + 1) * 3]
                ],
            }
...
def analyze_philosophy(text: str) -> dict:
    ...
    deductive_pat = re.findall(
        r"(?:all|every|no|if.*then|necessarily|must be|always)", text, re.IGNORECASE
    )
    inductive_pat = re.findall(
        r"(?:most|many|some|usually|typically|probably|likely|tends to|suggests)",
        text,
        re.IGNORECASE,
    )
    abductive_pat = re.findall(
        r"(?:best explanation|inference to the best|would explain|hypothesize|hypothesis)",
        text,
        re.IGNORECASE,
    )
    ...
    reasoning_type = (
        "deductive"
        if len(deductive_pat) > max(len(inductive_pat), len(abductive_pat))
        else "inductive"
        if len(inductive_pat) > len(abductive_pat)
        else "abductive"
    )
...
if __name__ == "__main__":
    ...
    if action == "analyze_text":
        text = (
            sys.argv[2] if len(sys.argv) > 2 else "This is a sample text for analysis."
        )
        ...
        text = (
            sys.argv[2]
            if len(sys.argv) > 2
            else "Artificial intelligence and machine learning are transforming industries."
        )
        ...
        text = (
            sys.argv[2]
            if len(sys.argv) > 2
            else "Chapter 1: Introduction to the subject matter."
        )
        ...
        text = (
            sys.argv[2]
            if len(sys.argv) > 2
            else "Descartes argues that because the mind is indivisible and the body is divisible, they must be distinct substances."
        )
        print(json.dumps(analyze_philosophy(text), indent=2))
    else:
        print(
            "Literature Module — available actions: analyze_text, extract_concepts, generate_study_guide, analyze_philosophy"
        )
