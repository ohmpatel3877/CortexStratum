...
COMPONENT_LIBRARY = {
    "arduino_uno": Component(
        ...
        pins={
            ...
            "A2": (20, 190, "ANALOG"),
            "A3": (20, 200, "ANALOG"),
            "A4": (20, 210, "ANALOG_I2C"),
            "A5": (20, 220, "ANALOG_I2C"),
            "5V": (0, 40, "POWER"),
            "3V3": (0, 50, "POWER"),
            "GND": (0, 60, "GROUND"),
            "VIN": (0, 70, "POWER_IN"),
...
class CircuitDesigner:
    ...
    def validate(self):
        ...
        for conn in self.circuit.connections:
            ...
            src_role = (
                src.pins.get(conn.from_pin, ("", "", ""))[2]
                if len(src.pins.get(conn.from_pin, (0, 0, ""))) > 2
                else ""
            )
            dst_role = (
                dst.pins.get(conn.to_pin, ("", "", ""))[2]
                if len(dst.pins.get(conn.to_pin, (0, 0, ""))) > 2
                else ""
            )
            if "POWER" in src_role and "GROUND" in dst_role:
                issues.append(
                    f"WARN: {conn.from_comp}.{conn.from_pin} (power) to {conn.to_comp}.{conn.to_pin} (gnd)"
                )
            if "GROUND" in src_role and "POWER" in dst_role:
                issues.append("OK: Ground reference established")
        return issues
    ...
    def generate_pin_map(self):
        ...
        for conn in self.circuit.connections:
            if "arduino" in conn.from_comp.lower():
                ...
                pins.append(
                    (conn.to_pin, conn.from_comp, conn.from_pin, conn.wire_type)
                )
        return pins
...
""")
        ...
        for i, (cname, comp) in enumerate(self.circuit.components.items()):
            ...
            svg_parts.append(
                f'  <rect x="{cx}" y="{cy}" width="{w}" height="{h}" rx="6" fill="#e8e8e8" stroke="#555" stroke-width="1.5"/>'
            )
            svg_parts.append(
                f'  <text x="{cx + w / 2}" y="{cy + 18}" text-anchor="middle" font-family="Arial" font-size="10" font-weight="bold" fill="#333">{cname}</text>'
            )
            svg_parts.append(
                f'  <text x="{cx + w / 2}" y="{cy + 30}" text-anchor="middle" font-family="Arial" font-size="8" fill="#666">{comp.type}</text>'
            )
            ...
            for j, (pname, (px, py, role)) in enumerate(comp.pins.items()):
                ...
                color = {
                    ...
                    "ANALOG": "#f39c12",
                    "I2C": "#8e44ad",
                }.get(role.split("_")[0], "#555")
                svg_parts.append(
                    f'  <circle cx="{cx + 10}" cy="{pin_y}" r="3" fill="{color}"/>'
                )
                svg_parts.append(
                    f'  <text x="{cx + 18}" y="{pin_y + 4}" font-family="monospace" font-size="8" fill="{color}">{pname}</text>'
                )
        ...
        for conn in self.circuit.connections:
            if conn.from_comp in self._placed and conn.to_comp in self._placed:
                ...
                pin_idx_src = (
                    list(src_pins.keys()).index(conn.from_pin)
                    if conn.from_pin in src_pins
                    else 0
                )
                pin_idx_dst = (
                    list(dst_pins.keys()).index(conn.to_pin)
                    if conn.to_pin in dst_pins
                    else 0
                )
                ...
                color = {"signal": "#3498db", "power": "#e74c3c", "gnd": "#333"}.get(
                    conn.wire_type, "#3498db"
                )
                svg_parts.append(
                    f'  <line x1="{x1 + 10}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="{color}" stroke-width="1.5" stroke-dasharray="4,2" opacity="0.7"/>'
                )
...
if __name__ == "__main__":
    ...
    for conn in cd.circuit.connections:
        print(
            f"  {conn.from_comp}.{conn.from_pin} -> {conn.to_comp}.{conn.to_pin} [{conn.wire_type}]"
        )
