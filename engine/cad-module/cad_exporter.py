...
def batch_export(scad_dir, output_dir, quality="medium", parallel=False):
    ...
    for i, scad in enumerate(scad_files):
        print(
            f"[{i + 1}/{len(scad_files)}] Exporting {scad.name}...", end=" ", flush=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input",
        nargs="?",
        default="G:/My Drive/Project/Bottle Flipper/cad",
        help="SCAD file or directory",
    )
    parser.add_argument("-o", "--output", default=None, help="Output directory")
    parser.add_argument(
        "-q", "--quality", default="medium", choices=["low", "medium", "high", "ultra"]
    )
    ...
    if input_path.is_dir():
        results = batch_export(
            input_path, args.output or input_path, args.quality, args.parallel
        )
    else:
